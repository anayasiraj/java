...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int numCat = StdIn.readInt();

        String[] catArray = new String[numCat];
        MenuNode[] menuArray = new MenuNode[numCat];

        for (int i = 0; i < numCat; i++){

            StdIn.readLine();
            String curCat = StdIn.readLine();
            catArray[i] = curCat;
            ...
            for (int j = 0; j < numDishes; j++){
                ...
                Dish dish = new Dish(curCat, dishName, ing);
                MenuNode menu = new MenuNode(dish, menuArray[i]);
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        int index = id % stockVarSize;   // what if not made idk
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        return ptr; // update the return value
    }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) { // *****
        ...
        node.getIngredient().setStockLevel(stock);
    ...
    public void updatePriceAndProfit() {
        ...
        MenuNode curCat = null; // 0 - 5 ig

        for (int i = 0; i < menuVar.length; i++ ){ // for every category down

            curCat = menuVar[i]; // 0 - 5

            while (curCat != null){ // while list at that index is not finished
                
                Dish curDish = curCat.getDish(); // dish in current node in list
                int[] arr = curDish.getStockID(); // list of ingredient ids of the dish
                double cost = 0;
                double price = 0;
                double profit = 0;

                for (int j = 0; j < arr.length; j++){ // for every ingredient id in a dish...
                    int id = arr[j];
                    Ingredient curIng = findStockNode(id).getIngredient();
                    cost += curIng.getCost(); // ... add the ingredient cost to the dish cost
                }

                price = cost * 1.2;
                profit = price - cost;

                curDish.setPriceOfDish(price);
                curDish.setProfit(profit);

                curCat = curCat.getNextMenuNode(); // go to next menu item to get next dish
    ...
    public void createStockHashTable (String inputFile){ // ********************************************
        ...
        while(StdIn.hasNextLine()){  
            ...
            Ingredient ingNode = new Ingredient(id, name, stock, cost);

            int index = id % stockVarSize;

            StockNode stockNode = new StockNode(ingNode, null);

            if (stockVar[index] != null){
                stockNode.setNextStockNode(stockVar[index]); 
            }

            // StockNode stockNode = new StockNode(ingNode, stockVar[index]);
            // stockNode.setNextStockNode(null) = new StockNode(ingNode, null);
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        if (transactionVar == null){ // if its the first transaction
            transactionVar = node;
        } else {
            TransactionNode prev = transactionVar;
            TransactionNode ptr = transactionVar.getNext();
            while (ptr != null){  // until ur on null node after last node (prev)
                prev = prev.getNext();
                ptr = ptr.getNext();
            }
            prev.setNext(node);
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        Dish dishM = findDish(dishName).getDish();
        int[] arr = dishM.getStockID();

        for (int i = 0; i < arr.length; i++){
            Ingredient curIng = findStockNode(arr[i]).getIngredient();
            if (curIng.getStockLevel() < numberOfDishes){
                return false;
            }
        }
    ...
    public void order (String dishName, int quantity){
        ...
        if (checkDishAvailability(dishName, quantity) == true){
            ...
            TransactionData data = new TransactionData("order", dishName, quantity, profit * quantity, true);
            addTransactionNode(data);
            int [] ids = dish.getStockID();
            for (int i = 0; i < ids.length; i++){
                int id = ids[i];
                StockNode node = findStockNode(id);
                String name1 = node.getIngredient().getName();
                updateStock(name1, id, -quantity);       ////////////////
            }
        } else {
            //Dish dish = findDish(dishName).getDish();
            ...
            MenuNode orig = findDish(dishName);
            MenuNode ptr = orig.getNextMenuNode();
            int index = findCategoryIndex(orig.getDish().getCategory());

            if (ptr == null){
                ptr = menuVar[index];
            }
            while (ptr != orig){
                String name = ptr.getDish().getName(); 
                if (checkDishAvailability(name, quantity) == true){ 
                    //order(name, quantity);
                    Dish dish2 = findDish(name).getDish();
                    double profit2 = dish2.getProfit();
                    TransactionData data2 = new TransactionData("order", name, quantity, profit2 * quantity, true);
                    addTransactionNode(data2);
                    int [] ids2 = dish2.getStockID();
                    for (int j = 0; j < ids2.length; j++){
                        int id2 = ids2[j];
                        StockNode node = findStockNode(id2);
                        String name2 = node.getIngredient().getName();
                        updateStock(name2, id2, -quantity);       ////////////////
                    }
                    break;
                } else {
                    TransactionData data3 = new TransactionData("order", name, quantity, 0, false);
    ...
    public void donation (String ingredientName, int quantity){
        ...
        int id = node.getIngredient().getID(); // lol
        int stock = node.getIngredient().getStockLevel();

        if (profit() > 50 && stock >= quantity){ // + check if enough stock????? are we donating? ig so yeah
            TransactionData data = new TransactionData("donation", ingredientName, quantity, 0, true);
            addTransactionNode(data);
            updateStock(ingredientName, id, -quantity);     /////////////
        } else {
    ...
    public void restock (String ingredientName, int quantity){
	    ...
	    StockNode node = findStockNode(ingredientName);
        int id = node.getIngredient().getID();
        double cost = node.getIngredient().getCost();
        double totalCost = cost * quantity;

        if (profit() >= totalCost){
            TransactionData data = new TransactionData("restock", ingredientName, quantity, -totalCost, true);
            addTransactionNode(data);
            updateStock(ingredientName, id, quantity);
        } else {
